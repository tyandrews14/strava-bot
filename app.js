  ...
  if (type === InteractionType.APPLICATION_COMMAND) {
    ...
    if (name === 'ridethewave') {
      const exampleEmbed = new EmbedBuilder()
        ...
        .setURL('https://strava.com/clubs/ocdurance')
        .setDescription('Join the club and run with the best')         
        .setTimestamp()
        .setFooter({ text: 'OCDurance' });
      return res.send({ 
      ...
      .then(token => {
          ...
          axios.get(`https://www.strava.com/api/v3/clubs/${process.env.STRAVA_CLUB_ID}/activities?page=1&per_page=1`, {
            ...
            if(lastActivityString === activityString) {
              ...
              let message
              console.log('New activity detected:', activityString)
              ...
              console.error('Strava API error:', error.response?.status, error.response?.data);
          });
